...
class UserAccessTest < ActionDispatch::IntegrationTest
  ...
  test "user destroy jobs" do
    ...
    admin = users(:admin)
    policy = Admin::JobPolicy.new({creator_id: manager.id}, user: manager)
    assert policy.apply(:destroy?)
    policy = Admin::JobPolicy.new({creator_id: admin.id}, user: manager)
    assert policy.apply(:destroy?)
  end
  ...
  test 'documents' do
    ...
    get reimbersment_documents_path, params: { trsc_id: trsc }
    assert_redirected_to root_path
    follow_redirect!
    assert_select 'div.alert', 'You are not authorized to access this page.'
