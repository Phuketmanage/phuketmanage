  ...
  def overusage
    ...
    unless last.nil?
      ...
      if amount.present? && last.amount.present? && (amount - last.amount).to_f / days > 1
        text = "Water overusage: #{amount}(#{date.strftime('%d.%m')}) - #{last.amount}(#{last.date.strftime('%d.%m')}) = #{amount - last.amount} in #{(date - last.date).to_i} days"
        Notification.create!(
          house_id: house_id,
          text: text
        )
      ...
      if amount_2.present? && last.amount_2.present? && (amount_2 - last.amount_2).to_f / days > 1
        text = "Water overusage: #{amount_2}(#{date.strftime('%d.%m')}) - #{last.amount_2}(#{last.date.strftime('%d.%m')}) = #{amount_2 - last.amount_2} in #{(date - last.date).to_i} days"
        Notification.create!(
          house_id: house_id,
          text: text
        )
