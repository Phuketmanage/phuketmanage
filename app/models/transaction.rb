...
class Transaction < ApplicationRecord
  ...
  scope :by_cat_for_owner,  lambda { |from, to|
                              joins(:balance_outs)
                                .where('date >= ? AND date <= ? AND for_acc = false', from, to)
                                .group(:type_id)
                                .select(
  ...
  scope :full_acc_for_owner, lambda { |from, to, owner_id|
                               where('date >= ? AND date <= ? AND hidden = false AND user_id = ?', from, to, owner_id)
                                 .order(:date, :created_at)
                             }
  scope :before_acc_for_owner, lambda { |from, owner_id|
                                 where('date < ? AND hidden = false AND user_id = ?', from, owner_id).all
                               }

  scope :for_salary, lambda { |type_id, start, finish|
                       joins({ booking: :house }, :balances)
                         .where(transactions: { type_id:, date: start..finish })
                         .where('bookings.start': ..finish)
                         .or(
                           where(transactions: { type_id:, date: ..(start.to_date - 1.day) })
  ...
  def write_to_balance(type, de_ow, cr_ow, de_co, cr_co)
    ...
    if types1.include?(type)
      ...
      booking.toggle_status unless booking.nil?
  ...
  def set_owner_and_house
    ...
    return unless house_id.nil? && !booking_id.nil?
